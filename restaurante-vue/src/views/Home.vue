...
<template>
  <div class="home">
   ...
   <body class="bg-fixed bg-cover bg-center bg-no-repeat h-full w-full lg:p-8"
    style="
      background-image: url(https://el-sabor-de-nuestra-tierra.000webhostapp.com/imagenes/restaurante/menu-art.jpg);
    "
   
   >
      <h1 class="text-4xl lg:p-0">{{titulo}}</h1>
      <div>
       <main>
      <form action="#" id="f1">
        ...
        <input type="number" name="cantidad" v-model="compra.cantidad" />
         <div>
          <input class="lg:mr-120 py-2 px-4"
            type="radio"
            name="envio"
        ...
        <div>
          <input class="lg:mr-120 py-2 px-4" type="radio" name="envio" v-model="compra.envio" value="1" />
          Sin Domicilio
        </div>
        <!-- <button @click.prevent="procesarInformacion" type="button" class="bg-gradient-to-r from-green-300 to-blue-400 hover:from-gray-400 hover:to-gray-800 text-gray-800 font-semibold lg:m-4 py-2 px-4 border border-gray-600 rounded shadow">
            AÑADIR +
      ...
      <div class="grid grid-cols-4 gap-y-10 sm:grid-cols-2 gap-x-6 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8 lg:p-8">
      <a 
      v-for="(unProducto, llave) in listaProductos"
      v-bind:key="llave"
      v-bind:value="llave"
...
export default {
  ...
  methods: {
    procesarInformacion(){
      ...
      if (this.compra.cantidad > 5) {
        this.compra.descuento = subtotal * 0.1;
      }
      else if (this.compra.subtotal>=200000) {
        this.compra.descuento = subtotal * 0.1;
      }

        if (this.compra.envio == 0) {
          valorEnvio = 0;
        }else if(this.compra.envio ==1){
          valorEnvio = 1;
        }
